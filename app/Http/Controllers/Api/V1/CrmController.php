...
class CrmController extends Controller
{
    ...
    public function followUps(Request $request) { $q = FollowUp::query()->with('assignedTo:id,name')->latest('due_date')->latest('due_time'); if ($request->user()->isSales()) $q->where('assigned_to', $request->user()->id); return $q->paginate(min($request->integer('per_page', 50), 100)); }
    public function storeFollowUp(Request $request) {
        $data = $request->validate(['subject_type' => ['required', 'in:lead,client'], 'subject_id' => ['required', 'integer'], 'type' => ['required', 'in:call,email,meeting,whatsapp,general'], 'due_date' => ['required', 'date'], 'due_time' => ['nullable', 'date_format:H:i'], 'timezone' => ['nullable', 'timezone'], 'note' => ['nullable', 'string']]);
